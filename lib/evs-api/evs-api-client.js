...
class EvsApiClient extends AbstractEvsApiClient {
    ...
    constructor(logger, server, protocol, port) {
        ...
        this.termCache = {};
        //TODO: load all the terms at the initial getgo from the FS cache?
        //TODO: invalidate and clean up cache after certain amount of time has passed.
    ...
    async _getRawConceptFromSVC(termID) {
        ...
        try {
            ...
            if (err.response && err.response.status == 404) {
                ...
                this.logger.error(`EVSApiClient: Bad Error fetching term ${termID}.  ${Object.keys(this.termCache).length} items in cache; ${this.fetchList.length} items fetchings`)
                console.log(err);
